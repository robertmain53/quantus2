...
- **Lightweight mobile navigation** – `SiteHeader` now computes `LightweightCalculator[]` (slug/fullPath/title/category/subcategory) and passes the top 10 to `MobileMenu`. The menu applies `.slice(0, 10)` plus a `View all` link so the client bundle stays small and no ISR overflow occurs.
