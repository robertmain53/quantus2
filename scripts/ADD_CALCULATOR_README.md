...
**Do NOT include the wrapper** (`component_type` + outer `config_json`). The script will handle the component type.
...
node scripts/add-calculator.js \
  ...
  --slug "/conversions/temperature/celsius-to-kelvin-converter" \
  --title "Convert Celsius to Kelvin" \
  --traffic 5000 \
  --date "11/28/2025" \
  --config data/configs/celsius-to-kelvin.json
...
### Missing arguments
Run `node scripts/add-calculator.js --help` to see all required fields.
