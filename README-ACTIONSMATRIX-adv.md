...
| **Content ops** | Keep the AI-driven workflow disciplined and adapt prompts by `content_type`. | 1. Version-control prompt templates in `scripts/generate-prompts.js` (tool template @ `scripts/generate-prompts/templates/tool.txt`, article template @ `.../article.txt`). 2. Automate CSV quality checks (`scripts/lint-configs.js`, schema guardrails). 3. Ensure `content_type` drives the prompt: `Tool` uses the calculator prompt, `Article` requests a 3k–5k word Ultimate Guide. 4. Regenerate `data/summary.json` nightly and validate slug coverage. | Prompt engineer + Automation engineer | Zero invalid rows; summary JSON row count = CSV active rows |
| **Navigation & trust** | Ensure every tool surface is discoverable and crawlable and that the dual taxonomy is surfaced. | 1. Build two mega menus: Discipline-focused (categories from `cat1.csv` with sub-levels) and Intent-focused (hubs/clusters from `cat2.csv`). 2. Ensure breadcrumbs/URLs follow `actionsmatrix.com/[hub]/[cluster]/slug` while discipline listing pages live at `/category/...`. 3. Maintain canonical metadata, sitemap, footers, disclaimers, and required analytics/verification tags globally. | SEO + Platform | No ISR page oversize warnings; sitemap coverage matches both taxonomies |
